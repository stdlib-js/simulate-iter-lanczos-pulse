...
var iterLanczosPulse = require( './../../dist' );


// FUNCTIONS //

function lanczos( t, A, tau ) {
	return A * sinc( (t*(2.0/(tau-1))) - 1.0 );
}
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterLanczosPulse, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which generates a "Lanczos" pulse waveform', function test( t ) {
	var expected;
	var actual;
	var delta;
	var tol;
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var iterLanczosPulse;
	var it1;
	var it2;
	var i;

	iterLanczosPulse = proxyquire( './../dist/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	it1 = iterLanczosPulse();
